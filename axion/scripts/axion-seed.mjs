...
try {
  ...
  for (const module of modules) {
    ...
    if (module === 'architecture' || modules[0] === module) {
      const rpbsContent = generateRPBS(ctx, upgradeCtx);
      const rebsContent = generateREBS(ctx, upgradeCtx);
      const registryDocs = {
        [`${axionRoot}/docs/product/RPBS_Product.md`]: rpbsContent,
        [`${axionRoot}/docs/product/REBS_Product.md`]: rebsContent,
      };

      for (const [filePath, content] of Object.entries(registryDocs)) {
        ensureFile(filePath, content, { overwrite: true });
      }

      const sourceDocsDir = path.join(axionRoot, 'source_docs', 'product');
      ensureFile(path.join(sourceDocsDir, 'RPBS_Product.md'), rpbsContent, { overwrite: true });
      ensureFile(path.join(sourceDocsDir, 'REBS_Product.md'), rebsContent, { overwrite: true });

      const workspaceRoot = path.resolve(axionRoot, '..');
      const overviewPath = path.join(workspaceRoot, 'PROJECT_OVERVIEW.md');
      if (projectIdea && fs.existsSync(overviewPath)) {
        const existing = fs.readFileSync(overviewPath, 'utf-8');
        if (existing.includes('No project description was provided') || existing.includes('UNKNOWN')) {
          const projectName = process.env.AXION_PROJECT_NAME || ctx.name;
          const lines = projectIdea.split('\n');
          const firstParagraph = lines.slice(0, Math.min(lines.length, 5)).join('\n').trim();
          const overviewContent = `# Project Overview: ${projectName}\n\n*Generated by AXION seed*\n\n**Stack Profile:** ${process.env.AXION_STACK_PROFILE || 'default-web-saas'}\n\n## What This Project Is\n\n${firstParagraph}\n\n## Additional Context\n\n${projectIdea}\n\n## Reading Guide\n\n1. Read this file first to understand what the project is about.\n2. Read \`axion/source_docs/product/RPBS_Product.md\` for the product brief.\n3. Read \`axion/source_docs/product/REBS_Product.md\` for engineering constraints.\n4. Then follow the domain reading order in \`AGENT_PROMPT.md\`.\n`;
          ensureFile(overviewPath, overviewContent, { overwrite: true });
          if (!jsonMode) console.log('Regenerated PROJECT_OVERVIEW.md with project idea');
        }
      }
    }
