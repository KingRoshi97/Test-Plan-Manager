...
const STATUS_COLORS: Record<string, string> = {
  ...
  applying: "bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300",
  verifying: "bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300",
  ...
  blocked: "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300",
        ...
        {musPolicy && (
          <div className="border rounded-lg p-4 border-[hsl(var(--border))] bg-[hsl(var(--card))]">
            ...
            <div className="space-y-1 text-xs">
              <div className="flex justify-between"><span className="text-[hsl(var(--muted-foreground))]">Apply consent required</span><span className={musPolicy.consent?.apply_required ? "text-green-600 font-medium" : "text-red-600"}>{musPolicy.consent?.apply_required ? "Yes" : "No"}</span></div>
              <div className="flex justify-between"><span className="text-[hsl(var(--muted-foreground))]">Publish consent required</span><span className={musPolicy.consent?.publish_required ? "text-green-600 font-medium" : "text-red-600"}>{musPolicy.consent?.publish_required ? "Yes" : "No"}</span></div>
        ...
        <tbody>
          {modes.map((m: any) => (
            <tr key={m.mode_id} className="border-t border-[hsl(var(--border))] hover:bg-[hsl(var(--accent)/0.5)]">
              ...
              <td className="px-3 py-2"><span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${m.execution_class === "manual_only" ? "bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300" : "bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300"}`}>{m.execution_class === "manual_only" ? "Manual" : "Scheduled OK"}</span></td>
              <td className="px-3 py-2 text-xs text-[hsl(var(--muted-foreground))]">{m.allowed_triggers?.join(", ")}</td>
              <td className="px-3 py-2">{m.hard_constraints?.no_apply ? <span className="text-xs text-red-600">Blocked</span> : <span className="text-xs text-green-600 font-medium">Allowed</span>}</td>
              <td className="px-3 py-2">{m.hard_constraints?.no_publish ? <span className="text-xs text-red-600">Blocked</span> : <span className="text-xs text-green-600 font-medium">Allowed</span>}</td>
          ...
          {gates.map((g: any) => (
            <div key={g.gate_rule_id} className="border rounded-lg p-4 border-[hsl(var(--border))] bg-[hsl(var(--card))]">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${g.status === "active" ? "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300" : "bg-gray-100 text-gray-800"}`}>{g.status}</span>
              </div>
              {g.predicate?.clauses && (
                <div className="mt-2">
            ...
            <tbody>
              {patches.map((p: any) => (
                <tr key={p.patch_type_id} className="border-t border-[hsl(var(--border))]">
                  ...
                  <td className="px-3 py-2"><span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${p.risk_class_default === "safe" ? "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300" : "bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300"}`}>{p.risk_class_default || "unknown"}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
            ...
            <div key={s.schedule_id} className="border rounded-lg p-4 border-[hsl(var(--border))] bg-[hsl(var(--card))]">
              <div className="flex items-center justify-between mb-2">
                ...
                <button
                  ...
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${s.status !== "disabled" ? "bg-green-500" : "bg-gray-300 dark:bg-gray-600"}`}
                >
    ...
    <div className="space-y-4">
      ...
      {showCreate && (
        <div className="border rounded-lg p-4 border-[hsl(var(--border))] bg-[hsl(var(--card))] space-y-3">
          ...
          {formError && <div className="text-sm text-red-600 bg-red-50 dark:bg-red-900/20 rounded p-2">{formError}</div>}
            ...
            <div key={run.run_id} className="border rounded-lg border-[hsl(var(--border))] bg-[hsl(var(--card))]">
              <div className="flex items-center justify-between px-4 py-3 cursor-pointer hover:bg-[hsl(var(--accent)/0.3)]" onClick={() => setExpandedRun(expandedRun === run.run_id ? null : run.run_id)}>
                <div className="flex items-center gap-3">
                  ...
                  <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_COLORS[run.status] || "bg-gray-100 text-gray-800"}`}>{run.status}</span>
              ...
              {expandedRun === run.run_id && (
                <div className="border-t border-[hsl(var(--border))] px-4 py-3 space-y-3">
                          ...
                          <tbody>
                            {run.units.map((u: any) => (
                              <tr key={u.unit_id} className="border-t border-[hsl(var(--border))]">
                                ...
                                <td className="px-2 py-1"><span className={`inline-flex px-1.5 py-0.5 rounded text-xs ${u.status === "done" ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300" : u.status === "failed" ? "bg-red-100 text-red-700" : "bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400"}`}>{u.status}</span></td>
                                ...
                                <td className="px-2 py-1">
                                  {u.verification_results?.length > 0 ? u.verification_results.map((v: any, i: number) => (
                                    <span key={i} className={`inline-flex items-center gap-0.5 ${v.passed ? "text-green-600" : "text-red-600"}`}>
                  ...
                  <div className="flex gap-2 pt-2 border-t border-[hsl(var(--border))]">
                    ...
                    {runAction.error && <span className="text-xs text-red-600">{(runAction.error as Error).message}</span>}
                  </div>
                </div>
              )}
            </div>
