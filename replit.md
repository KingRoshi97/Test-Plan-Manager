...
- `planner.ts` — Derives build plan from kit: stack profile, repo shape, 8 ordered build slices (scaffold → types_contracts → data_layer → api_routes → components → integration → tests → config). Generates feature pages per-feature (auth, dashboard, settings, analytics, profile detection via `classifyFeature()`), UI primitives (Button, Card, Input, Modal, Table, LoadingSpinner), layout components (AppLayout, Header, Sidebar), workflow-based middleware, NotFound page, and globals.css
- `workspace.ts` — Creates/manages `Axion/.axion/runs/RUN-XXXXXX/build/` with repo/, manifests, verification report, zip
- `generator.ts` — Core code generation engine: reads all 12 doc slots into `KitContext` (requirements, design, architecture, implementation, security, quality, ops, data, api_contracts, release, governance, analytics) plus work breakdown and page manifest. Smart context selection via `findRelevantDocs()` scores docs by relevance to each file's role and feature. Role-specific prompt construction for app_entry (full routing manifest), feature_page, auth_page, settings_page, layout_component, ui_component, api_route, db_schema, test, hook. Deterministic generators for main.tsx (BrowserRouter), NotFound, LoadingSpinner, globals.css (Tailwind), package.json (includes react-router-dom). Per-file heartbeat logging prevents stall detection kills during long builds
