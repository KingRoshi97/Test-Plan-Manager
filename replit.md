...
- `planner.ts` — Derives build plan from kit: stack profile, repo shape, 8 ordered build slices (scaffold → types_contracts → data_layer → api_routes → components → integration → tests → config). **Enterprise depth**: per-feature sub-components ({Feature}Form, {Feature}List, {Feature}Detail, barrel index) via `featureNeedsForm/List/Detail/Search` heuristics; auth infrastructure (AuthContext, ProtectedRoute, useAuth hook, AuthLayout, ForgotPassword); shared services (api/endpoints, api/interceptors, validators/index + per-feature validators, store); UI primitives (LoadingSpinner, ErrorBoundary, EmptyState, Pagination, SearchInput); per-feature unit + integration tests; workflow-based middleware. Feature classification via `classifyFeature()` for pages (auth, dashboard, settings, analytics, profile, search, content, notification). Repo shape includes auth/validators/store subdirs
- `workspace.ts` — Creates/manages `Axion/.axion/runs/RUN-XXXXXX/build/` with repo/, manifests, verification report, zip
- `generator.ts` — Core code generation engine: reads all 12 doc slots into `KitContext` plus work breakdown and page manifest. Smart context selection via `findRelevantDocs()`. **Enterprise generators**: deterministic for ErrorBoundary, EmptyState, Pagination, ApiEndpoints (per-feature CRUD routes), AuthContext (JWT auth provider), ProtectedRoute (role-based guard), ForgotPassword page, vite.config.ts (React plugin + @ alias), tailwind.config.ts (content paths + theme), postcss.config.js (tailwind + autoprefixer), index.html (Vite entry), plus existing main.tsx/NotFound/LoadingSpinner/globals.css/package.json (includes zod, @tanstack/react-query, clsx, axios, lucide-react, date-fns, react-hook-form; devDeps: @vitejs/plugin-react). AI-assisted roles with rich prompt context: feature_form, feature_list, feature_detail, feature_validation, auth_hook, auth_layout, api_interceptor, state_store, validation. System prompt includes available UI components, restricted package list, file manifest for correct imports, BrowserRouter-in-main.tsx instruction, and auth infrastructure for cross-file awareness. `extractCodeBlock()` handles fenceless blocks, multiple blocks (picks largest), and strips stray fence markers. Token budget: 8192 for complex roles (feature_page, auth_page, app_entry, layout_component, feature_form/list/detail, settings_page), 6144 for standard roles
