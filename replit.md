...
- `packagingEnforcement.ts` — Packaging preflight: reconciles manifest targets against actual bundle outputs, verifies all hard gates passed from build_quality_report.json, checks package-critical artifacts exist (6 required), verifies proof ledger and trace map. Produces `packaging_decision.json` with block_reasons, gate_evidence, manifest_mismatches, artifact_checks.
...
**BAQ Packaging Enforcement**: Preflight gate is embedded directly in `packager.ts:packageKit()` — any caller of `packageKit` is subject to preflight enforcement. Preflight runs AFTER kit build to reconcile against actual bundle. Decision propagated to `build_quality_report.json` via `updateQualityReportWithPackagingDecision()`.

**BAQ Test Coverage**: 79 unit tests in `test/unit/baq.enforcement.test.ts` covering: all 7 gate evaluations (pass/fail/edge), evaluateAllGates (packaging_eligible), quality report (decision taxonomy, coverage/inventory metrics, score calculation, packaging decision propagation), failure report (retry classification, collector aggregation, summary counts), preflight checks (missing/invalid artifacts), file tracking (unplanned/placeholder detection), reconciliation (variance computation), packaging enforcement (gate blocking, missing artifacts, proof/trace checks, manifest mismatches, false-green prevention, packager-level enforcement), hook runner (upstream deps, pass-through, error catching), and regression cases (missing infra/shared contracts/proof, incomplete feature families, manifest mismatch, false-green path).
