...
- `generator.ts` — Core code generation engine: reads all 12 doc slots into `KitContext` plus work breakdown and page manifest. Smart context selection via `findRelevantDocs()`. **Enterprise generators**: deterministic for ErrorBoundary, EmptyState, Pagination, ApiEndpoints (per-feature CRUD routes), AuthContext (JWT auth provider), ProtectedRoute (role-based guard), ForgotPassword page, plus existing main.tsx/NotFound/LoadingSpinner/globals.css/package.json (now includes zod, @tanstack/react-query, clsx). AI-assisted roles with rich prompt context: feature_form, feature_list, feature_detail, feature_validation, auth_hook, auth_layout, api_interceptor, state_store, validation. System prompt includes available UI components, packages, and auth infrastructure for cross-file awareness. Token budget: 8192 for complex roles (feature_page, auth_page, app_entry, layout_component, feature_form/list/detail, settings_page), 6144 for standard roles
