...
**BAQ Test Coverage**: 73 unit tests in `test/unit/baq.enforcement.test.ts` covering: all 7 gate evaluations (pass/fail/edge), evaluateAllGates (packaging_eligible), quality report (decision taxonomy, coverage/inventory metrics, score calculation), failure report (retry classification, collector aggregation, summary counts), preflight checks (missing/invalid artifacts), file tracking (unplanned/placeholder detection), reconciliation (variance computation), packaging enforcement (gate blocking, missing artifacts, proof/trace checks, manifest mismatches, false-green prevention), hook runner (upstream deps, pass-through, error catching), and regression cases (missing infra/shared contracts/proof, incomplete feature families, manifest mismatch, false-green path).
