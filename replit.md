...
### Roshi v2.1 (Documentation & Repo Hygiene System)
Roshi is the documentation and repo hygiene system that powers Axiom Assembler - now implemented as command-line dev tooling.

**Protected Paths** (`roshi/protected-paths.json`):
- Core files that hygiene scripts cannot touch: `server/db/schema.ts`, `migrations/**`, `src/theme/**`, `roshi/**`, `scripts/**`, `assembler/**`
- All hygiene commands enforce fail-fast via `assertNoProtectedTouches()`

**Hygiene Commands** (all support `--dry-run`):
- `npm run roshi:reorg` - Archive root clutter to `_archive/legacy_misc/<timestamp>/`
- `npm run roshi:consolidate` - Consolidate `attached_assets/` and scattered `.md` into `docs/legacy/**`
- `npm run roshi:reset` - Archive workspace, rebuild skeleton, regenerate `roshi/domains.json`
- `npm run roshi:purge` - Remove non-active legacy files (only after consolidation + verify passes)

**Model B Control Plane** (`scripts/lib/project-overview-parse.mjs`):
- Deterministic domain extraction from `PROJECT_OVERVIEW.md`
- Single parsing module enforced across all tools
- Generates `roshi/domains.json` with version, source, and domain list

**Verify Gates** (`npm run roshi:verify`):
- **Sources Gate**: Validates `sources.json`, ensures Project Overview is first (Model B contract)
- **Terminology Gate**: Checks `terminology.md` has required sections
- **Build Order Gate**: DFS cycle detection, full coverage required (UNKNOWN not valid bypass)

**Registry Templates** (`docs/roshi_v2/00_registry/`):
- `terminology.md` - Term definitions with required headings
- `DOMAIN_BUILD_ORDER.md` - Domain prerequisite graph
- `PURGE_POLICY.md` - Retention rules per asset type

**Run Sequences** (`docs/roshi_v2/RUN_SEQUENCES.md`):
1. Bootstrap: `roshi:init` → `roshi:gen` → `roshi:verify`
2. Rebuild: `roshi:reset` → `roshi:gen` → `roshi:verify`
3. Wipe-rebuild: `roshi:reset --wipe-frontend` → full pipeline
4. Docs-only: `roshi:verify` → `roshi:gen`
