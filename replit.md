...
**Integration**: BAQ hooks wired into `Axion/src/core/build/runner.ts` — runs `onBuildAuthorityLoaded` → `onKitExtractionStart` → `onKitExtractionComplete` → `onDerivedInputsBuild` before legacy extraction. BAQ artifacts written to `{runDir}/build/baq_kit_extraction.json` and `{runDir}/build/baq_derived_build_inputs.json` (distinct from legacy `kit_extraction.json` to prevent overwrite). **Hard blocking**: BAQ validation failures, gate failures (G-BQ-01, G-BQ-02), and blocking hook failures all throw errors that halt the build — builds cannot proceed past extraction without structurally valid artifacts.
...
- Upgrade tab: Full revision-controlled upgrade workbench (`App/src/components/upgrade/UpgradeTabShell.tsx`) with 12 sub-components supporting the full lifecycle (baseline → session → plan → execute → compare → verify → promote/rollback). Components: UpgradeHeaderRail (status overview strip), UpgradeSessionLauncher (start new upgrade session with mode/objective/scope), CurrentUpgradeSessionCard (active session status), RevisionTimeline (horizontal revision history), UpgradeTabSwitcher (Plan/Workspace/Compare/Verify/Promote sub-tabs), UpgradePlanPanel (AI-generated plan with findings/changes/risks/rollback), UpgradeWorkspacePanel (execution progress and candidate artifacts), RevisionComparePanel (diff viewer with artifact/config/structural/semantic views), UpgradeVerificationPanel (check results with required/optional/compatibility groups), UpgradePromotionPanel (promote candidate or rollback with confirmation), UpgradeActionFooter (contextual navigation). Shared sub-components in `App/src/components/upgrade/shared.tsx` (RevisionStatusChip, SessionStatusChip, UpgradeModeBadge, VerificationVerdictCard, DiffStatStrip, RollbackTargetCard, formatTimeAgo). DB: 11 tables (assembly_revisions, upgrade_sessions, upgrade_plans, upgrade_plan_changes, upgrade_session_artifacts, revision_diffs, revision_diff_items, revision_verifications, revision_verification_checks, revision_events, revision_snapshots). API: 20+ endpoints under `/api/assemblies/:id/upgrade*` and `/api/assemblies/:id/upgrades/*` in `server/upgrade-routes.ts`. Types: `shared/upgrade-types.ts` with 10 upgrade modes (MM-05 through MM-19), all status enums, and full prop contracts
...
### Control Planes
- **ICP (Internal Control Plane)** — `Axion/src/core/controlPlane/`: Run orchestrator (api.ts), model/store (model.ts, store.ts), policies (policies.ts), releases (releases.ts), pins (pins.ts), audit (audit.ts), state machine governor (stateMachine.ts). States: CREATED → QUEUED → RUNNING → PAUSED/GATED → (FAILED | COMPLETED | CANCELLED) → ARCHIVED. RunController delegates all transitions to StateMachineGovernor. Supports pause/resume/cancel/retry. Store has optimistic locking (version field). Audit has actor identity, action categories (lifecycle/governance/evidence), hash chain verification. Pins support scope (run/global) and immutable pin class. Releases support signer authorization and policy checks before publish. PolicyEngine supports stage-aware evaluation, rich comparison operators, and policy snapshots per run. CLI wired via RunController.
