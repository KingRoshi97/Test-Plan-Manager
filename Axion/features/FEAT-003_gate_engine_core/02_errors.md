# FEAT-003 — Gate Engine Core: Error Codes
  ...
  |------|----------|---------|-----------|--------|
  | `ERR-GATE-001` | error | Gate Engine Core initialization failed | false | Check configuration and dependencies. |
| `ERR-GATE-002` | error | Gate Engine Core invalid input | false | Validate input against schema before passing. |
| `ERR-GATE-003` | warning | Gate Engine Core degraded operation | false | Review logs for root cause. |
