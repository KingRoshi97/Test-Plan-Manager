...
describe("BAQ Quality Score Calculation", () => {
  ...
  it("scores maximum 100 with all passing artifacts", () => {
    ...
    const report = buildQualityReport({
      ...
      buildId: "BUILD-001",
      extraction: makeExtraction(),
      derivedInputs: makeDerivedInputs({ summary: { ...makeDerivedInputs().summary, derivation_completeness: 100 } } as any),
      inventory: makeInventory(),
  ...
  it("approved_with_warnings when score below 70 but all gates pass", () => {
    ...
    const report = buildQualityReport({
      ...
      derivedInputs: makeDerivedInputs({ summary: { ...makeDerivedInputs().summary, derivation_completeness: 30 } } as any),
      inventory: makeInventory(),
